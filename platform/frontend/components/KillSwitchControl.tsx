...
export default function KillSwitchControl({ onStatusChange }: KillSwitchControlProps) {
  ...
  const fetchStatus = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/api/kill-switch/status`,
        { credentials: 'include' }
      )
  ...
  const handleActivate = async () => {
    ...
    try {
      setActing(true)
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/api/kill-switch/activate`,
        {
          method: 'POST',
  ...
  const handleDeactivate = async () => {
    ...
    try {
      setActing(true)
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5001'}/api/kill-switch/deactivate`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
