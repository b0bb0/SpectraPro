...
Generate the scan intent JSON now:`;
   ...
   * SQLi tags now added based on parameter detection, not just DB tech
   */
  private getFallbackIntent(context: AssetContext, profile: ScanProfile): AIScanIntent {
    ...
    // Discover parameters first to inform SQLi assessment
    ...
    // Add SQLi tag if parameters detected (mandatory)
    if (hasParameters) {
      tags.push('sqli');
   ...
   * Parameters are now MANDATORY trigger - database detection only refines confidence
   */
  private assessSQLiFeasibility(
    ...
    // Parameters MUST trigger SQLi testing regardless of DB detection
    if (hasDynamicParams) {
      likely = true;
