...
# Update config with tunnel ID (handles both fresh and re-runs)
if grep -q "TUNNEL_ID_HERE" ./cloudflared/config.yml; then
  sed -i '' "s/TUNNEL_ID_HERE/${TUNNEL_ID}/" ./cloudflared/config.yml
  echo -e "  ${GREEN}✓${NC} Config updated with tunnel ID"
else
  # Replace any existing tunnel ID with the current one
  sed -i '' "s/^tunnel: .*/tunnel: ${TUNNEL_ID}/" ./cloudflared/config.yml
  echo -e "  ${GREEN}✓${NC} Config updated with tunnel ID (replaced existing)"
fi
echo ""
...
for i in $(seq 1 30); do
  if docker compose -f docker-compose.local.yml exec -T backend echo "ready" &>/dev/null; then
    break
  fi
  sleep 2
done
docker compose -f docker-compose.local.yml exec -T backend npx prisma migrate deploy 2>/dev/null && \
  ...
  echo -e "  ${YELLOW}!${NC} Migrations may need manual run: docker compose -f docker-compose.local.yml exec backend npx prisma migrate deploy"
